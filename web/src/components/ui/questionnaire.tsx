...
/**
 * shadcn's Questionnaire (the radix-nova registry item), over @shadcn/react.
 *
 * Put here by hand: the registry has no item for this project's style, and
 * its source is written for the CLI to fill in. What differs from it is size
 * alone — it is drawn for a finger on a phone (44px rows, 16px text), and
 * this is a column in a desktop window, so rows, text and buttons are the
 * sizes the rest of the panel uses. What it does is the package's.
 */
...
function QuestionnaireTitle({
  ...
  return (
    <QuestionnairePrimitive.Title
      ...
      className={cn(
        "text-sm leading-snug font-medium text-pretty [&:not(:has(~[data-slot=questionnaire-description]))]:mb-3",
        className
      )}
      {...props}
    />
...
function QuestionnaireDescription({
  ...
  return (
    <QuestionnairePrimitive.Description
      ...
      className={cn("text-sm text-pretty text-subtle-foreground", className)}
      {...props}
    />
  )
}
  ...
  return (
    ...
    >
      <QuestionnairePrimitive.Input
        ...
        className={cn(
          "h-8 w-full min-w-0 rounded-md border border-input bg-transparent px-2.5 py-1 text-sm transition-[color,box-shadow,background-color] outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:cursor-not-allowed disabled:bg-input/50 disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:bg-input/30 dark:disabled:bg-input/80 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40",
