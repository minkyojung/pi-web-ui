...
/** A card log written straight to disk, the way the accept test seeds a history. */
const seedCards = (path, events) => {
  mkdirSync(join(cwd, ".pi/cards"), { recursive: true });
  writeFileSync(join(cwd, `.pi/cards/${path}.jsonl`), events.map((e) => JSON.stringify(e)).join("\n") + "\n");
};

it("노트를 열면 카드 목록이 함께 오고, 카드는 지금 자리로 온다", async () => {
  const quote = "고양이는 밤에 잘 본다";
  writeFileSync(join(cwd, "card.md"), `머리말\n\n${quote}\n\n맺음말\n`);
  clear();
  send({ type: "open_note", path: "card.md" });
  const note = await want("note", (m) => m.path === "card.md");
  assert.deepEqual((await want("cards", (m) => m.path === "card.md")).cards, []);
  const from = note.text.indexOf(quote);
  seedCards("card.md", [
    { kind: "opened", id: "c1", at: 1, from, to: from + quote.length, log: history("card.md").length, quote, question: "왜 그렇지" },
    { kind: "answered", id: "c1", at: 2, text: "빛을 더 모으기 때문이다.", sessionId: "s1", entryId: "e1" },
  ]);
  // 카드가 생긴 뒤에 그 위를 고치면, 카드는 밀린 자리로 온다.
  clear();
  send({ type: "save_note", path: "card.md", text: note.text.replace("머리말", "머리말을 길게 고쳤다"), base: note.modified });
  const moved = await want("cards", (m) => m.path === "card.md");
  const [card] = moved.cards;
  const now = readFileSync(join(cwd, "card.md"), "utf8");
  assert.equal(now.slice(card.from, card.to), quote, "카드는 고른 글 위에 그대로 있다");
  assert.equal(card.answer.text, "빛을 더 모으기 때문이다.");
  assert.equal(card.placed, undefined);
});

it("카드의 답을 노트에 넣으면 pi의 글로 들어가고, 두 번 눌러도 한 번만 들어간다", async () => {
  clear();
  send({ type: "place_card", path: "card.md", card: "c1" });
  const changed = await want("note_changed", (m) => m.path === "card.md" && m.changes.some((c) => c.author === "pi"));
  const written = changed.changes.find((c) => c.author === "pi");
  assert.deepEqual([written.sessionId, written.entryId], ["s1", "e1"], "카드에 담긴 대화와 턴이 그대로 기록된다");
  const text = readFileSync(join(cwd, "card.md"), "utf8");
  assert.ok(text.includes("고양이는 밤에 잘 본다\n\n빛을 더 모으기 때문이다."), "고른 글 바로 아래에 제 문단으로");
  assert.equal((await want("cards", (m) => m.cards[0]?.placed)).cards[0].placed, true);
  clear();
  send({ type: "place_card", path: "card.md", card: "c1" });
  send({ type: "open_note", path: "card.md" });
  const again = await want("note", (m) => m.path === "card.md");
  assert.equal(again.text.split("빛을 더 모으기 때문이다.").length - 1, 1, "두 번 들어가지 않는다");
});

it("해결한 카드는 해결로 남고, 지운 카드는 사라진다", async () => {
  clear();
  send({ type: "resolve_card", path: "card.md", card: "c1" });
  assert.equal((await want("cards", (m) => m.cards[0]?.resolved)).cards[0].resolved, true);
  clear();
  send({ type: "delete_card", path: "card.md", card: "c1" });
  assert.deepEqual((await want("cards", (m) => m.path === "card.md" && m.cards.length === 0)).cards, []);
});

it("이름을 바꾸거나 휴지통에 갔다 와도 카드는 노트를 따라간다", async () => {
  const quote = "따라오는 글";
  writeFileSync(join(cwd, "follow.md"), `${quote}\n`);
  clear();
  send({ type: "open_note", path: "follow.md" });
  await want("note", (m) => m.path === "follow.md");
  seedCards("follow.md", [{ kind: "opened", id: "f1", at: 1, from: 0, to: quote.length, log: history("follow.md").length, quote, question: "왜" }]);
  clear();
  send({ type: "rename_note", path: "follow.md", to: "followed.md" });
  await want("note_renamed", (m) => m.to === "followed.md");
  send({ type: "open_note", path: "followed.md" });
  assert.equal((await want("cards", (m) => m.path === "followed.md")).cards[0].id, "f1");
  clear();
  send({ type: "delete_note", path: "followed.md" });
  const gone = await want("note_deleted", (m) => m.path === "followed.md");
  assert.equal(existsSync(join(cwd, ".pi/cards/followed.md.jsonl")), false, "카드도 휴지통으로 간다");
  clear();
  send({ type: "restore_note", trashed: gone.trashed, path: "followed.md" });
  await want("note_created", (m) => m.path === "followed.md");
  send({ type: "open_note", path: "followed.md" });
  assert.equal((await want("cards", (m) => m.path === "followed.md")).cards[0].id, "f1", "되살리면 카드도 돌아온다");
});

it("카드로 물으면 카드가 먼저 생기고, 답은 카드에만 담기며 노트는 그대로다", async (t) => {
  const quote = "물은 100도에서 끓는다";
  writeFileSync(join(cwd, "askcard.md"), `${quote}\n\n다음 문단\n`);
  clear();
  send({ type: "open_note", path: "askcard.md" });
  const note = await want("note", (m) => m.path === "askcard.md");
  clear();
  send({ type: "prompt", text: "한 문장으로 왜 그런지 알려줘.", note: "askcard.md", ask: { id: 11, path: "askcard.md", from: 0, to: quote.length, card: "k1" } });
  // 답을 기다리는 동안에도 카드는 이미 있다: 질문과 인용을 들고, 답 없이.
  const opened = (await want("cards", (m) => m.path === "askcard.md" && m.cards.length === 1)).cards[0];
  assert.deepEqual([opened.quote, opened.question, opened.answer], [quote, "한 문장으로 왜 그런지 알려줘.", undefined]);
  const done = await want("ask_done", (m) => m.id === 11, 180_000);
  const refused = inbox.find((m) => m.type === "message_end" && m.message?.stopReason === "error");
  if (done.outcome !== "written" && refused) {
    t.diagnostic(`모델이 답하지 못했다 — ${String(refused.message.errorMessage).slice(0, 160)}`);
    return t.skip("이 기계의 모델이 답하지 못했다");
  ...
  assert.equal(done.outcome, "written");
  const answered = (await want("cards", (m) => m.cards[0]?.answer)).cards[0];
  assert.ok(answered.answer.text.length > 0);
  assert.ok(answered.answer.sessionId && answered.answer.entryId, "어느 대화의 어느 턴인지 카드가 들고 있다");
  assert.equal(readFileSync(join(cwd, "askcard.md"), "utf8"), note.text, "노트는 한 글자도 안 바뀐다");
});

it("답을 기다리다 멈추면 카드에 그렇게 남고, 노트에는 아무것도 안 들어간다", async () => {
  const quote = "다음 문단";
  clear();
  send({ type: "prompt", text: "이건 멈출 물음이다.", note: "askcard.md", ask: { id: 12, path: "askcard.md", from: 0, to: quote.length, card: "k2" } });
  await want("cards", (m) => m.cards.some((c) => c.id === "k2"));
  const before = readFileSync(join(cwd, "askcard.md"), "utf8");
  clear();
  send({ type: "abort" });
  const stopped = (await want("cards", (m) => m.cards.some((c) => c.id === "k2" && c.failed))).cards.find((c) => c.id === "k2");
  assert.equal(stopped.failed, "interrupted");
  assert.equal(stopped.answer, undefined);
  assert.equal(readFileSync(join(cwd, "askcard.md"), "utf8"), before, "노트는 그대로다");
  assert.equal((await want("ask_done", (m) => m.id === 12)).outcome, "interrupted");
});
