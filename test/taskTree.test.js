...
test("서 있는 자리: 도는 것이 먼저, 끝난 것, 다음 것, 나머지", () => {
  assert.equal(standingOf({ number: "2.1", done: false }, { running: "2.1", next: "2.1" }), "running");
  assert.equal(standingOf({ number: "2", done: false }, { running: "2.1", next: "2.1" }), "running", "a heading runs by way of its sub-task");
  assert.equal(standingOf({ number: "1", done: true }, { running: "2.1", next: "2.2" }), "done");
  assert.equal(standingOf({ number: "2.2", done: false }, { running: null, next: "2.2" }), "next");
  assert.equal(standingOf({ number: "3", done: false }, { running: null, next: "2.2" }), "todo");
  assert.equal(standingOf({ number: "21", done: false }, { running: "2.1", next: null }), "todo", "21 is not 2's sub-task");
});
