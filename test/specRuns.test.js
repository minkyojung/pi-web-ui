...
const mark = (task) => ({ spec: "email-auth", task, title: `Task ${task}`, done: [] });

test("작업의 실행은 세션에서 읽는다 — 표식이 있는 세션만, 오래된 것부터, 표식과 세션 id와 함께", async (t) => {
  const f = folder(t);
  f.session(null, "just talking");
  const one = f.session(mark("1"), "The door is in.\nChecks: none");
  await new Promise((r) => setTimeout(r, 20));
  const two = f.session(mark("2.1"), "The board is cut.");
  const runs = await taskRuns(f.cwd, f.sessionDir);
  assert.deepEqual(runs.map((run) => [run.spec, run.task, run.title, run.session]), [
    ["email-auth", "1", "Task 1", one],
...
test("심사 중은 작업마다 가장 새 실행이고, 커밋이 그 세션을 이름하지 않은 것 — 받아들인 뒤 다시 돌리면 다시 심사 중, 받아들인 것을 다시 열면 아니다", () => {
  const run = (task, session, at) => ({ spec: "email-auth", task, title: "", session, at });
