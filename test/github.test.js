...
const answer = JSON.stringify({
	data: {
		repository: {
			b0: { associatedPullRequests: { nodes: [{ number: 31, state: "OPEN", url: "https://github.com/x/y/pull/31", isDraft: false, reviewDecision: null, headRefName: "me/scripts", commits: { nodes: [{ commit: { statusCheckRollup: { contexts: { nodes: [{ status: "COMPLETED", conclusion: "SUCCESS" }, { status: "COMPLETED", conclusion: "FAILURE" }] } } } }] } }] } },
...
test("GitHub's answer is read by branch: the latest pull request, its checks counted, a branch without one left out", () => {
	...
	assert.deepEqual(prs.get("me/scripts"), { number: 31, state: "OPEN", url: "https://github.com/x/y/pull/31", draft: false, review: "", checks: { total: 2, pending: 0, failed: 1 } });
	assert.deepEqual(prs.get("me/spec"), { number: 26, state: "MERGED", url: "https://github.com/x/y/pull/26", draft: true, review: "APPROVED", checks: { total: 0, pending: 0, failed: 0 } });
