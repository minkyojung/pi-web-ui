...
test("the query asks for exactly these branches, each aliased in order, with the branch name quoted as GraphQL wants it", () => {
	...
	assert.match(query, /mergeStateStatus additions deletions\n\s+commitCount: commits \{ totalCount \}/, "an alias: `commits` is asked for once more, for the last one's checks");
});
