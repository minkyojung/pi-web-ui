...
test("#태그는 공백 뒤나 줄 머리에서, 글자·숫자·_·-·/로 이어진다", () => {
  assert.deepEqual(tagsIn("#a b #b_c-d/e f #한글"), ["#a", "#b_c-d/e", "#한글"]);
});

test("숫자만으로는 태그가 아니다 — Obsidian의 규칙", () => {
  assert.deepEqual(tagsIn("#1 #2024 #a1"), ["#a1"]);
});

test("헤딩, 낱말 속, 코드, URL 속의 #은 태그가 아니다", () => {
  assert.deepEqual(tagsIn("# heading\n"), []);
  assert.deepEqual(tagsIn("a#b"), []);
  assert.deepEqual(tagsIn("`#x`"), []);
  assert.deepEqual(tagsIn("see https://x.y/page#frag now"), []);
});
