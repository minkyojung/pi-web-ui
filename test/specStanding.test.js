...
import { docPath, docStanding, docTitle, speaksFor, standingOf, standingWord, stateWords, waitingLine, waitingSpec } from "../web/src/specStanding.ts";

/** A spec as the server describes it (SpecInfo). */
const spec = (name, { approved = 0, waiting = null, waitingAt = waiting ? 1000 : null, written = waiting ? [waiting] : [] } = {}) => ({
  name,
  approved,
  waiting,
  waitingAt,
