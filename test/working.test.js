...
import { agentLine, currentStep, glyphOf, lastRun, nextUnseen, resultSeen, taskLabel, taskTitle } from "../web/src/working.ts";
...
test("작업의 실행이면 줄이 어느 작업인지 들고 간다 — 단계는 그대로", () => {
  const task = { task: "2.2", title: "Paint it" };
...
test("줄에는 번호, 제목은 title에 — 제목은 제일 길고 제일 먼저 잘린다", () => {
  const task = { task: "2.2", title: "Paint it" };
  assert.equal(taskLabel(task), "Task 2.2");
  assert.equal(taskTitle(task), "Task 2.2 · Paint it");
});
