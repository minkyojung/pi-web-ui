 ...
 *
 * What a task changed is a commit (spec.ts makes one at a task's end), and
 * reading it is the other half of specResults.ts, which lists them. Before
...
export async function readCommit(root: string, name: string): Promise<CommitRead | null> {
	...
	const head = await run(root, ["show", "-s", `--format=${["%H", "%h", "%s", "%ct", "%P", trailer("Spec"), trailer("Task"), trailer("Checks")].join(FIELD)}`, `${name}^{commit}`, "--"]);
	if (head === null) return null;
	const [commit = "", short = "", title = "", seconds = "", parents = "", spec = "", task = "", checks = ""] = head.toString("utf8").replace(/\n+$/, "").split(FIELD);
