...
### Changed
- With the agent's column folded away, its half of the strip at the foot of the window folds into the context ring alone, and the ring says what the agent is doing: an arc travels round it while the agent works, an amber dot means it is waiting for your answer, a blue dot means a run finished while you were not looking and stays until you open the column, and a dashed ring means the window is offline. Point at the ring and it opens out into the full line; click it and the column comes back. With reduced motion on, the travelling arc stays put and fades in and out instead.
...
- The strip at the foot of the window is divided where the window is: what is about the note stays under the note, and under pi there is now what pi is doing — a spinner and the step it is on, the tool it reached for and what it touched, or that it is thinking — so a run can be watched without looking away from what you are writing. pi's tool mode and the context ring moved down there too, out of the row under the message box. Drag the divider and the strip follows it.
...
- Narrowing pi's column no longer leaves the row under the message box overlapping itself. It gives things up in order instead: the model's name shortens. Everything comes back as you widen it.
...
- Web access is a switch of its own in the tool menu, under the three modes: pi searches and reads the web in every mode, and turning it off turns off all four web tools at once — and keeps them off when you change mode.
