...
- While a task runs, the strip at the foot of the window says which: `Task 2.2 · Paint it — edit auth.ts · then 3`, the step it is on and the tasks queued after it. In `tasks.md` the ▶ of that task turns, and so does the heading's it is under.
