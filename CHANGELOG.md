...
- A task of a spec can be started from its line: point at a task in `tasks.md` that is still to do and a ▶ appears to its left — press it and that task runs, exactly as `/spec-run` would, in a session of its own. Not on a heading whose sub-tasks are the work, nor on a task already done. It is greyed, and says why, while the agent is working or the spec is not yet approved to the end.
