...
- What a spec's tasks came to is at the foot of the window: `✓ 5 tasks`, and `· 2 new` beside it when some have finished since you last looked. Press it for the list — each task with its commit, how much it changed, when, and what the agent said it checked (or `no checks`, which is the one worth opening); a task run again shows its last run and how many times. Opening the list is looking: what was new stops being new.
- A line of that list opens what the task changed, as a page of its own: the commit and its sums at the head, then every file it touched one after another, read-only and coloured as a file is — the lines taken out in red, the lines put in in green, the words that changed within a line marked, and everything unmodified folded away with three lines kept either side of a change. Press a fold to open it where it is. A binary file, or one too large to compare, says so rather than being drawn. A file's name opens the file as it is now.
