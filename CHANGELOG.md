...
- A spec's requirements are written to be judged by. The agent reads the code the work touches first and says what is there now; each requirement is a plain sentence in your language with criteria a test or a look can show to be true or false — no "As a user, I want…", no WHEN/THEN/SHALL, headings in your language too; what the work will not do is listed; and what only you can decide is put to you at the end, with the options, the one it recommends, and what it assumed meanwhile. It used to fill a form from your one line, without looking at the code.
